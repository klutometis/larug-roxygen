...
* TODO Translation roclet
  ...
  #+BEGIN_SRC R :tangle translate.R :shebang #!/usr/local/bin/R -f
    ...
    detect.source <- function(query, google.key=getOption('google.key'))
      fromJSON(getURL(sprintf('https://www.googleapis.com/language/translate/v2/detect?key=%s&q=%s',
                              google.key,
                              curlEscape(query))))[[1]]$detections[[1]][[1]]$language
    
    detect.source('hello world')
